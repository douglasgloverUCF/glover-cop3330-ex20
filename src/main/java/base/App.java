...
public class App {
    ...
    public static void main(String[] args) {
        ...
        double tax = myApp.getTax();

        myApp.getTotal(order, tax);
    }
    double getOrder()
    {
    ...
    void getTotal(double order, double tax)
    {
        double taxTotal = order * tax;
        double total = order + taxTotal;

        String message;
        if (tax == 0)
            message = String.format("The total is $%.2f.", total);
        else
            message = String.format("The tax is $%.2f.\nThe total is $%.2f.", taxTotal, total);
